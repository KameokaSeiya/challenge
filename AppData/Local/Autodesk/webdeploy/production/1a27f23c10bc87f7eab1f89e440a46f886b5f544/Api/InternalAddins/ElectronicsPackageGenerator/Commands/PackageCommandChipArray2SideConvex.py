...
class PackageCommandChipArray2SideConvex(PackageCommand.PackageCommand):
    ...
    def on_input_changed(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs, changed_input: adsk.core.CommandInput, input_values: dict ):
        ...
        if changed_input.id == 'horizontalPadCount':
            ...
            pad_count = int(inputs.itemById('horizontalPadCount').selectedItem.name)    
            if self.ui_data['horizontalPadCount'] != pad_count:
                self.non_parametric_update = True if self.ui_data['horizontalPadCount'] == 4 or pad_count == 4 else False
