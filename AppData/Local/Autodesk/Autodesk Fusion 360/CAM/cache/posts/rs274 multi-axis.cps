...
groupDefinitions = {
  multiaxis: {title:"Multi-axis", description:"Multi-axis post options", order: 0},
  preferences: {title:"Preferences", description:"Preferences", order: 1},
  general: {title:"General settings", description:"General settings", order: 2},
  header: {title: "Header", description: "NC file header", order: 3},
  operation: {title: "Operation", description: "Operation properties", order: 10}
};
...
function activateMachine() {
  ...
  setFeedrateMode(); // TAG: move call to defineMachine when supported in GUI
...
/**
  Define the multi-axis feedrate mode
*/
function setFeedrateMode(reset) { // setting reset to 'true' will reset to the default multi-axis feedrate mode
  // don't need to set multi-axis feedrates for TCP machines
  if ((tcpIsSupported && !reset) || !machineConfiguration.isMultiAxisConfiguration()) {
    return;
  }
  machineConfiguration.setMultiAxisFeedrate(
    tcpIsSupported ? FEED_FPM : FEED_INVERSE_TIME, // can be FEED_DPM
    9999.99, // maximum output value for inverse time feed rates
    INVERSE_MINUTES, // can be INVERSE_SECONDS or DPM_COMBINATION for DPM feeds
    0.5, // tolerance to determine when the DPM feed has changed
    1.0 // ratio of rotary accuracy to linear accuracy for DPM calculations. Could be "unit == MM ? 1.0 : 0.1" for certain machines
  );
...
function defineMachine() {
  ...
  if (true) { // hardcoded machine configuration takes precedence
    ...
    var aAxis = createAxis({coordinate:X, table:true, axis:[1, 0, 0], offset:[0, 0, 0], range:[-120, 30], cyclic:false, preference:-1, tcp:false});
    var cAxis = createAxis({coordinate:Z, table:true, axis:[0, 0, 1], offset:[0, 0, 0], cyclic:true, tcp:false});
...
function getWorkPlaneMachineABC(workPlane, rotate) {
  ...
  if (rotate && !currentSection.isOptimizedForMachine()) {
    machineConfiguration.setToolLength(compensateToolLength ? currentSection.getTool().getBodyLength() : 0); // define the tool length for head adjustments
    currentSection.optimize3DPositionsByMachine(machineConfiguration, abc, tcpIsSupported ? 0 : 1);
  }
  return abc;
}
...
function onCyclePoint(x, y, z) {
  ...
  if (currentSection.isOptimizedForMachine()) {
    forward = machineConfiguration.getOptimizedPosition(currentSection.workPlane.forward, currentMachineABC, TCP_XYZ, OPTIMIZE_BOTH, false);
  } else {
