/**
  ...
  $Revision: 43334 f7d7f466fec188908a7a678dc1a68d270338f3ba $
  $Date: 2021-06-24 16:47:00 $
...
var yFormat = createFormat({decimals: 3});
...
var dimensionFormat = createFormat({decimals: 3, zeropad: false, suffix: "mm"});
...
function onOpen() {
  if (unit == IN) {
    writeComment("Outputting the code in millimeter, as the printer does not support switching unit via G20/G21.");
    unit = MM;
  }
  getPrinterGeometry();
  ...
  // set unit, does not cause error, but it is ignored by the firmware apparently.
