/**
  ...
  $Revision: 43325 2f0db559b8471ed5ad24de3a501f1846e83407b7 $
  $Date: 2021-06-22 06:59:54 $
...
var cFormat = createFormat({decimals:4, forceDecimal:true, scale:DEG, cyclicLimit:Math.PI * 2});
...
function invertAxes(activate, polarMode) {
  ...
  if (getProperty("cAxisDesignation")) {
    _prefix = getSpindle(true) == SPINDLE_SUB ? "B" : "C";
  } else {
    _prefix = getSpindle(true) == SPINDLE_SUB ? "CB" : "CM";
  }
  cOutput = createVariable({prefix: _prefix}, cFormat);
  if (polarMode) {
    cOutput.disable();
    yOutput = createVariable({prefix: _prefix}, yFormat);
  }
}
